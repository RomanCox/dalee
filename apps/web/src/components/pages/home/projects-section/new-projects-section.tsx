...
export const ProjectsSection = memo(({slidesData}: ProjectsSectionProps) => {
  ...
  const [slides, setSlides] = useState<TProject[]>(slidesWithCopiedElements);
  ...
  const slideMods = useCallback((id: number) => {
    return {
  ...
  const isSliderInitializedRef = useRef(false);

  useEffect(() => {
    if (!windowSize.width) return;

    if (!isSliderInitializedRef.current) {
      isSliderInitializedRef.current = true;
    ...
    <section id="projects" className={styles.section}>
      <div className={styles.container}>
        ...
        <Text
          ...
          title={"ПРОЕКТЫ"}
          titleTag={"h2"}
          addTitle={`(${(slidesData.length - 1).toString().padStart(2, "0")})`}
